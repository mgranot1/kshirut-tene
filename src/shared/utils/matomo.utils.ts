import { bootstrap, event } from "matomo-client";
...
export const setUserDetails = (user: IUserUnit) => {
  ...
  if(isMatomoInit) 
  {
    ...
    _paq.push(["setCustomVariable", 3, "emergencyLevelDesc", user.emergencyLevelDesc]);
    _paq.push(["setCustomVariable", 4, "routineLevelDesc", user.routineLevelDesc]);
...
export const trackPageView = (path: string, title: string) => {
    ...
    if (lastTrackedPath==path && lastTrackedTime && (now - lastTrackedTime) < 1000) {
      return;
    }
...
export const matomoEvent = (action: string, category: MatomoCategory, data: string, value: number) => {
